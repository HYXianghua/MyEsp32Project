...
// //wifi回调
// static esp_err_t xMainWifiEventHandler(void *ctx, system_event_t *event)
// {
//     switch (event->event_id)
//     {
//     case SYSTEM_EVENT_STA_START:
...
static void example_wifi_init(void)
{
    ...
    // ESP_ERROR_CHECK(esp_event_loop_init(xMainWifiEventHandler, NULL));
    // wifi_init_config_t cfg = WIFI_INIT_CONFIG_DEFAULT();
    // ESP_ERROR_CHECK(esp_wifi_init(&cfg));
    // ESP_ERROR_CHECK(esp_wifi_set_storage(WIFI_STORAGE_RAM));
...
void example_espnow_data_prepare(example_espnow_send_param_t *send_param)
{
    ...
    buf->seq_num = 0XBAAB;
    s_example_espnow_seq[buf->type]++;
    buf->crc = 0;
    buf->magic = send_param->magic;
    /* Fill all remaining bytes after the data with random values */
    //esp_fill_random(buf->payload, send_param->len - sizeof(example_espnow_data_t));
    for (uint8_t i = 0; i < send_param->len - sizeof(example_espnow_data_t); i++)
    {
        buf->payload[i] = 0XAB;
    }
...
static void example_espnow_task(void *pvParameter)
{
    ...
    ESP_LOGI(TAG, "开始发送广播数据"); //Start sending broadcast data
    ...
    while (xQueueReceive(s_example_espnow_queue, &evt, portMAX_DELAY) == pdTRUE)
    {
        ...
        case EXAMPLE_ESPNOW_SEND_CB:
        {
            ...
            ESP_LOGD(TAG, "发送数据 to " MACSTR ", status1: %d", MAC2STR(send_cb->mac_addr), send_cb->status);
            ...
            ESP_LOGI(TAG, "发送数据 to " MACSTR "", MAC2STR(send_cb->mac_addr));
